import ast
import csv
with open('list_of_the_LFY_regulated_genes_from_prediction_with_expression_data_with_non_conserved_BS.txt', 'r') as f:
...
with open('tab_sep_LFY_regulated_genes_from_database.csv', 'rb') as csvfile:
	...
	for line in reader:
		if line[4] == "FC_35sLFY_LER" :
			...
			if (float(line[49]) < 1 and float(line[51]) < 0.05) or (float(line[49]) > 1 and float(line[53]) < 0.05) :
