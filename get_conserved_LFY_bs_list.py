...
def get_LFY_binding_sites(matScore,matRev,FastaFile,dependency_matrix,threshold,factorTranscription):
...
list_of_the_LFY_binding_sites = get_LFY_binding_sites(matScore,matRev,FastaFile,dependency_matrix,threshold,factorTranscription)

print("len(list_of_the_LFY_binding_sites) : ",len(list_of_the_LFY_binding_sites))
...
def add_modified_sequences_from_accessions():
	vcf_reader = vcf.Reader(filename='VCF_files/1001genomes_snp-short-indel_only_ACGTN.vcf.gz')
...
text_file.write(str(list_of_the_LFY_binding_sites))
text_file.close()
